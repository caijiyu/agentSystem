...
<style type="text/css">
	...
	<div class="input-group">
		...
		class="${cssClass}" style="${cssStyle}"/>
       		 ...
       		 <input id="${id}Name"  name="${labelName}" ${allowInput?'':'readonly="readonly"'}  type="text" value="${labelValue}" data-msg-required="${dataMsgRequired}"
		class="form-control singleGraph height-control rightAngle" style="${cssStyle}" />
       		 <span class="input-group-btn">
	       		 <button type="button"  id="${id}Button" class="btn btn-info leftAngle height-control">选择
	             </button> 
       		 </span> 
...
<script type="text/javascript">
	$("#${id}Button, #${id}Name").click(function(){
		...
		if ($("#${id}Button, #${id}Name").hasClass("disabled")){
			return true;
		}
		// 正常打开	
		layer.open({
		    ...
		    yes: function(index, layero){ //或者使用btn1
						...
						$("#${id}Name").focus();
						if($("#${id}Name").val() != ""){
							$("#${id}Name-error").css({display:'none'});
						}
						layer.close(index);
				    	       },
