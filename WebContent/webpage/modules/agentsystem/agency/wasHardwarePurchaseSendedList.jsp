...
<body>
			...
			<div class="card-body">
				<div class="tab-content">
						<div class="row col-9" style="float: right;">
							<form action="${ctx}/hardware/hardwarePurchase/sendedList" method="post" style="width: 100%" id="searchForm">
							...
							<c:forEach items="${page.list}" var="wasHardwarePurchase">
								<tr>
									...
									<td class="tableControl">
										<shiro:hasPermission name="agency:wasHardwarePurchase:sendedView">
										...
										<shiro:hasPermission name="agency:wasHardwarePurchase:sendedLogistics">
											<button type="button" data-toggle="tooltip" data-original-title="物流" class="btn btn-info btn-circle btn-xs" onclick="logistics('${wasHardwarePurchase.expressCoding}','${fn:escapeXml(wasHardwarePurchase.expressNumber)}',this)">
												<i class="iconf icon-tubiao"></i>
											</button>
										</shiro:hasPermission> 
										<shiro:hasPermission name="agency:wasHardwarePurchase:sendedConfirm">
										...
										<shiro:hasPermission name="agency:wasHardwarePurchase:sendedDelete">
				...
				<div class="modal-body" style="overflow-y:scroll; max-height: 600px;">
					<form>
						...
						<div class="form-group row">
							...
							<div class="col-8 divMiddle">
								<div id="expressNumber" class="divFont" style="white-space: pre-wrap;word-break: break-all;"></div>
							</div>
						</div>
	...
	<div class="modal fade" id="logisticsModal" data-backdrop="static">
		<div class="modal-dialog">
			...
			<div class="modal-content">
				<!--对话框头-->
				<div class="modal-header" style="float: right; display: block; background-color: #f4f5f9; border-top-left-radius: 5px; border-top-right-radius: 5px;">
					<span style="font-size: 14px">物流</span>
					<button type="button" class="close" data-dismiss="modal" aria-hidden="true">×</button>
				</div>
				<!--主体部分-->
	...
	<script type="text/javascript">
		...
		function confirm(hardwarePurchaseId,sendState) {
			layer.confirm("确认当前货物已妥投？", {closeBtn: 0, icon : 3 ,skin : 'layui-layer-molv',btn: ['确认','取消']}, 
					function(){
						var index = layer.load(1, {shade: [0.1,'#fff']});
						$.ajax({
		...
		//查看物流
		function logistics(expressCoding, expressNumber,obj) {
			$(obj).blur();
			var index = layer.load(1, {shade: [0.1,'#fff']});
			$.ajax({
			    url:'${ctx}/hardware/hardwarePurchase/logistics',
			    type:'POST',
			    data:{
