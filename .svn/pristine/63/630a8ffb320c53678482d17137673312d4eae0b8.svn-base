...
<c:if test="${not empty content}">
	...
	<c:if test="${hideType != '1'}">
		<div id="messageBox" class="alert alert-${ctype}" style="font-size: 14px;padding-left: 65px;color: red;">${content}</div>
	</c:if> 
	<c:if test="${hideType != '0'}">
		<script type="text/javascript">if(!top){top=1;top("${content}","${ctype}",{persistent:true,opacity:0});$("#messageBox").show();}</script>
	</c:if>
</c:if>