							...
							<c:forEach items="${page.list}" var="agentLevel">
								<tr>
									...
									<td class="tableControl"><shiro:hasPermission
											...
											<button id="azx" type="button" data-toggle="tooltip"
												onclick="lookOver(${agentLevel.agentLevelId},this)" data-original-title="查看"
												class="btn btn-info btn-circle btn-xs">
												<i class="fa fa-search-plus"></i>
											</button>
										</shiro:hasPermission> <shiro:hasPermission name="sysdata:wasAgentLevel:fap">
											<button type="button" data-toggle="tooltip"
											onclick="distributePolicy(${agentLevel.agentLevelId},this)" data-original-title="分配销售方针"
												class="btn btn-info btn-circle btn-xs">
...
<div class="modal fade" id="modalB" data-backdrop="static">
	...
	//分配销售方针列表展示
	function distributePolicy(id,obj){
		$(obj).blur();
		$("#ida").val(id);
		...
		$.ajax({
			...
			success : function(data){
				$.each(data,function(i,li){
					var str='<option id="'+li.id+'roleNameIdo'+'" value="'+li.id+'"></option>';
					$("#inlineFormCustomSelect").append(str);
					$('#'+li.id+'roleNameIdo').text(li.name);
				}
				 );
			},
		});
		...
		function lookOver(id,obj){
			...
			$.ajax({
				...
				success : function(data){
					...
					}else if(data[0].delegateAgent==1){
						$("#cashaa").hide();
					...
					if(data[2].length>0){
						for(var i=0;i<data[2].length;i++){
							var str='<tr><td id="'+i+'wakceHaco'+'">'
								    +'</td></tr>'
								    $("#tables").append(str);
								    $('#'+i+'wakceHaco').text(data[2][i]);
						} 
					}
