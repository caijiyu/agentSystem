...
<body>
				...
				<div class="tab-pane p-20 active" id="home2" role="tabpanel">
					...
					<div class="row col-9" style="float: right;">
						<form action="${ctx}/agentSystem/Software/list" method="post" style="width: 100%" id="searchForm">
							...
							<input autocomplete="off" type="text" class="form-control height-control col-3" placeholder="请输入软件名称关键字" id="keywords" value="${fn:escapeXml(name)}" style="float: right;"> 
							<input type="hidden" id="keyword" value="${fn:escapeXml(name)}"  name="name" />
								...
								<tr>
									...
									<td class="tableControl">
										...
										<shiro:hasPermission name="sysdata:wasSoftware:edit">
											<button type="button" data-toggle="tooltip" onclick="show(${software.softwareId},${software.price},this)" data-original-title="编辑" class="btn btn-info btn-circle btn-xs">
	...
	$('#keywords').bind('keypress',function(event){
		...
		function search(){//查询，页码清零
			$("#pageNo").val(0);
		    $("#keyword").val($("#keywords").val().replace(/(^\s+)|(\s+$)/g, "")); 
			$("#searchForm").submit();
		}
		//分类搜索
