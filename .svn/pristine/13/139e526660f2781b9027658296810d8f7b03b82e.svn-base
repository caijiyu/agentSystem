			...
			<div class="col-9" style="float: right;">
				<form action="${ctx}/sysdata/wasHardware/list" method="post"
					...
					<input autocomplete="off" type="text"
						...
						id="hardWareNamea" style="float: right;" value="${fn:escapeXml(log.name)}"> 
						<input id="hardWareNameb" name="name" type="hidden" value="${fn:escapeXml(log.name)}" /> 
...
<script type="text/javascript">
		...
		$('#hardWareNamea').bind('keypress', function(event) {
			if (event.keyCode == "13") {
				search();
			}
		});
		...
		function confirm(obj){
			...
			if (isValid) {
				if(Number(totalMoney)>=Number(butPrice)){
					...
					var a = (hardwareAccount - butPrice).toFixed(2);
					if(a>=0){
						...
						var vipSpend = (Number($("#buyPrice").val())-Number($("#hardwareAccount").val())).toFixed(2);
		...
		var countNumber = "${fn:length(page.list)}";
		//删除硬件(逻辑删除)
